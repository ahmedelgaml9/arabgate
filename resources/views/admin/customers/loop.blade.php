...
@endif
<td>
    <a class="btn btn-default btn-rounded btn-sm" href="{{ url('controll/customers/'.$row->id.'/edit') }}" ><span class="fa fa-pencil"></span></a>
<!--        <button class="btn btn-danger btn-rounded btn-sm" onClick="delete_row('trow_{{ $row->id }}');"><span class="fa fa-times"></span></button>-->
    {!! Form::open(['action'=>['CustomersController@destroy',$row->id],'method'=>'delete' ,'style'=>'display: inline']) !!}
