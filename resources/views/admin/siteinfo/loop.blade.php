...
<tr id="trow_{{ $row->id }}">
    ...
    <td><strong>{{  $row->desc }}</strong></td>
     ...
     <td>
        <a class="btn btn-default btn-rounded" href="{{ url('controll/siteinfo/'.$row->id.'/edit') }}" ><span class="fa fa-pencil"></span></a>
 <!--        <button class="btn btn-danger btn-rounded btn-sm" onClick="delete_row('trow_{{ $row->id }}');"><span class="fa fa-times"></span></button>-->
        {!! Form::open(['action'=>['SiteinfoController@destroy',$row->id],'method'=>'delete' ,'style'=>'display: inline']) !!}
