...
//=========== add by my ==========//
use App\Http\Requests\CountrylinksRequest;  //============ request for this controller ==========//
use App\Countrylinks;   //========= admin model =============//
use App\Countries;

class CountrieslinksController extends Controller {

    //=======  request and model and view file =============//
    public function __construct(CountrylinksRequest $request, Countrylinks $model) {
        $this->request = $request;
        $this->model = $model;
        $this->view = 'admin/countrieslinks/';
    }
    ...
    public function index() {
        if (is_null($this->request->value)) {
            ...
            $this->request->flash();
            $rows = Slide::where('title', 'like', "%{$this->request->value}%")
                    ->paginate(25);
        }
