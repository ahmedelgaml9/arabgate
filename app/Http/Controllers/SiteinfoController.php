...
use App\Siteinfo;   //========= admin model =============//
class SiteinfoController extends Controller {

    //=======  request and model and view file =============//
    public function __construct(Request $request, Siteinfo $model) {
        $this->request = $request;
        $this->model = $model;
        $this->view = 'admin/siteinfo/';
    }
    ...
    public function update($id) {
        ...
        if(!isset($this->request['projects']))
        {
           $array = array_add($this->request->all(), 'projects', 0);
        }
         if(!isset($this->request['portfolio']))
        {
           $array = array_add($this->request->all(), 'portfolio', 0);
        }
