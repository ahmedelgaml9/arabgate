...
//=========== add by my ==========//
use App\Http\Requests\AboutusRequest;  //============ request for this controller ==========//
use App\Aboutus;   //========= admin model =============//

class AboutusController extends Controller {

    //=======  request and model and view file =============//
    public function __construct(AboutusRequest $request, Aboutus $model) {
        $this->request = $request;
        $this->model = $model;
        $this->view = 'admin/aboutus/';
    }
    ...
    public function index() {
        if (is_null($this->request->value)) {
            ...
            $this->request->flash();
            $cats = Aboutus::where('title', 'like', "%{$this->request->value}%")
                    ->paginate(25);
        }
        $cats->setPath('aboutus');
        if ($this->request->ajax()) {
            return response()->json(view($this->view . 'loop', compact('cats'))->render());
        }
