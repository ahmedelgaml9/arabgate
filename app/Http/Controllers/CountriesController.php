...
use App\Countries;  //========= admin model =============//

class CountriesController extends Controller {

    //=======  request and model and view file =============//
    public function __construct(Request $request, Countries $model) {
    ...
    public function index() {
        if (is_null($this->request->value)) {
            ...
            $this->request->flash();
            $services = Slider::where('title', 'like', "%{$this->request->value}%")
                    ->paginate(25);
        }
        $services->setPath('countries');
